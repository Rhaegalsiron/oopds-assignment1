...
void ThinkingRobot::thinkRobot()
{
    ...
    {   
        ...
        {   
            ...
            if (this->unit->moveModule != NULL && this->unit->moveModule == "STEALTH_BOT") //if we need to get the type i think might need add something in other class, so i didnt touch it
            {
                int moveIndex = rand() % 2 + 1;
                switch (moveIndex)
                {
                case 1: this->unit->moveModule->useAbility(); break;
                case 2: this->unit->defaultModule->move(randomDirection); break;
                }
            } else{
                this->unit->defaultModule->move(randomDirection); //here i am not sure how u call the move function
            };
        }
        //LOOK
        else if (action == "look")
        {   
            ...
            srand(static_cast<unsigned>(time(0)));
            if (this->unit->seeingModule != NULL && this->unit->seeingModule == "MAP_VISION_BOT")
            {   
                int seeIndex = rand() % 2 + 1;
                switch (seeIndex)
                {
                case 1: this->unit->seeingModule->useAbility(); break;
                case 2: this->unit->defaultModule->look(newX, newY); break;
                }
                ...
                switch (seeIndex)
                {
                case 1: this->unit->seeingModule->useAbility(newX, newY); break;
                case 2: this->unit->defaultModule->look(newX, newY); break;
                }
            } else{
        ...
        {   
            ...
            srand(static_cast<unsigned>(time(0)));
            if (this->unit->fireModule != NULL && this->unit->fireModule != "LONG_SHOT_BOT" && this->unit->fireModule != "SEMI_AUTO_BOT")
            {
                int fireIndex = rand() % 2 + 1;
                switch (fireIndex)
                {
                case 1: this->unit->fireModule->useAbility(); break;
                case 2: this->unit->defaultModule->fire(newX, newY); break;
                }
            } else{
